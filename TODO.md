...
## Phase 4 — 검색 (요구사항 3·6) ✅ 완료

- [x] 프로젝트 목록 화면 — 전역 검색(모든 프로젝트), 결과에 프로젝트명 표시
- [x] 프로젝트 상세 화면 — SearchOverlay(현재 프로젝트 검색)
- [x] 자재 이름 + 별칭(alias) 부분 일치(LIKE), 디바운스 200ms
- [x] 결과 탭 → 해당 도형 중앙 정렬 + 하이라이트 + (자재면) 바텀시트
- [x] **완료 기준**: 두 화면 모두에서 자재명/별칭으로 찾아 해당 위치/자재로 이동.

> features/search 신설(types/repositories/hooks/ui). 전역 이동은 `?focus=shapeId` 라우트 파라미터,
> 상세 진입 후 캔버스가 중앙 정렬. 공용 SearchBar 추가.
...
## Phase 6 — 학습 모드 (위치·이름 맞히기) ✅ 완료

- [x] 학습 진입 시 자재명 캡션 가림(공간 라벨은 유지)
- [x] 위치 맞히기: 자재 이름 제시 → 캔버스 도형 탭 → 정답/오답 피드백
- [x] 이름 맞히기: 도형 하이라이트 → 객관식 보기 선택 → 피드백
- [x] 진행도·점수·완료 화면·다시하기
- [x] **완료 기준**: 두 방식 퀴즈 동작.

> features/learn 신설(types/libs/stores/hooks/ui). Viewer "학습" FAB → 유형 선택 시트.
> 같은 이름이 여러 도형에 있으면 모두 정답. 편집 진입/화면 이탈 시 학습 종료.

---

## 참고 — Phase 7

PRD 로드맵은 Phase 6(학습 모드)까지다. 별도 Phase 7 범위는 PRD에 정의돼 있지 않아
Phase 1~6 으로 전체 로드맵을 완료했다. (PHASE_WORKFLOW 제목의 "1~7" 은 일반 표현)
